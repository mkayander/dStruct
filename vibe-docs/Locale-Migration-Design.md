...
6. ~~Session / device hints~~ — `LocaleAppLayout` passes proxy `ssrDeviceType` into the theme; session via `getServerSession` → `SessionGate` (P10; next-auth v4 single mount).
