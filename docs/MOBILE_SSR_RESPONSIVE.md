...
| `src/app/(default-locale)/playground/layout.tsx` | App Router default-locale playground: reads `headers()` via `resolveSsrDeviceType`, passes `ssrDeviceType` into `LocaleAppLayout` → `AppShellProviders`.                                                                  |
| `src/app/[lang]/playground/layout.tsx`           | Same SSR device hint for locale-prefixed playground (`/de/playground`, …).                                                                                                                                                |
| `src/proxy.ts`                                   | Sets `Accept-CH` / `Vary` on playground paths via `applyDeviceHintResponseHeaders` (Edge-safe helper in `ssrDevice.ts`).                                                                                                  |
