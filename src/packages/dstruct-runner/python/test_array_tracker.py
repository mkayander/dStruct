...
from array_tracker import TrackedArray, create_tracked_array
from typing import List, Dict, Any

class TestTrackedArray(unittest.TestCase):
    def setUp(self) -> None:
        self.callstack: List[Dict[str, Any]] = []

    def test_basic_array_creation(self):
        arr, self.callstack = create_tracked_array([1, 2, 3], "test_array")
    ...
    def test_nested_arrays(self):
        matrix, self.callstack = create_tracked_array([[1, 2], [3, 4]], "matrix")
        matrix[0][1] = 5
        _ = matrix[1][0]
    ...
    def test_array_slicing(self):
        arr, self.callstack = create_tracked_array([1, 2, 3, 4, 5], "test_array")
        _ = arr[1:3]
        arr[1:3] = [6, 7]
    ...
    def test_multiple_arrays(self):
        arr1, callstack1 = create_tracked_array([1, 2, 3], "arr1")
        arr2, callstack2 = create_tracked_array([4, 5, 6], "arr2")
        arr1[0] = arr2[1]
    ...
    def test_array_with_none_values(self):
        arr, self.callstack = create_tracked_array([None, 1, None], "test_array")
        arr[0] = 2
        self.assertEqual(len(self.callstack), 2)
        self.assertEqual(self.callstack[1]["name"], "addArrayItem")
    ...
    def test_array_with_mixed_types(self):
        arr, self.callstack = create_tracked_array([1, "hello", 3.14, True], "test_array")
        arr[1] = "world"
        self.assertEqual(len(self.callstack), 2)
        self.assertEqual(self.callstack[1]["name"], "addArrayItem")
    ...
    def test_empty_array(self):
        arr, self.callstack = create_tracked_array([], "empty")
        self.assertEqual(len(self.callstack), 1)
        frame = self.callstack[0]
