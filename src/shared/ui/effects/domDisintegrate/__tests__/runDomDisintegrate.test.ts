...
vi.mock("#/shared/ui/effects/domDisintegrate/buildDisintegrateCapture", () => ({
  buildDisintegrateCapture: vi.fn(),
}));

vi.mock(
  "#/shared/ui/effects/domDisintegrate/buildChunkMaskSequenceAsync",
  () => ({
    buildChunkMaskSequenceAsync: vi.fn(),
  }),
);

const { buildDisintegrateCapture } =
  await import("#/shared/ui/effects/domDisintegrate/buildDisintegrateCapture");
const { buildChunkMaskSequenceAsync } =
  await import("#/shared/ui/effects/domDisintegrate/buildChunkMaskSequenceAsync");
const { runDomDisintegrate } =
...
describe("runDomDisintegrate", () => {
  ...
  it("builds a fast capture when no snapshot is available", async () => {
    ...
    vi.mocked(buildDisintegrateCapture).mockResolvedValue({
      sourceCanvas: null,
      particles: [createParticle()],
      displayWidth: 120,
      displayHeight: 48,
    ...
    await expect(
      runDomDisintegrate(element, { origin: { x: 60, y: 24 } }),
    ).resolves.toBeUndefined();
    expect(buildDisintegrateCapture).toHaveBeenCalledWith(element, {
      mode: "fast",
  ...
  it("throws when capture produces no particles", async () => {
    ...
    ).rejects.toBeInstanceOf(DomDisintegrateError);
  });
  ...
  it("applies radial masks while chunk masks are building when origin is set", async () => {
    const syncElementWaveMask =
      await import("#/shared/ui/effects/domDisintegrate/syncElementWaveMask");
    ...
    const animation = runDomDisintegrate(element, {
      captureSnapshot: snapshot,
      origin: { x: 60, y: 24 },
      maskMode: "chunks",
      maxDuration: 0.2,
  ...
  it("keeps radial masks when chunk sequence arrives after radial fallback started", async () => {
    const syncChunkMaskSequence =
      await import("#/shared/ui/effects/domDisintegrate/syncChunkMaskSequence");
    const syncElementWaveMask =
      await import("#/shared/ui/effects/domDisintegrate/syncElementWaveMask");
